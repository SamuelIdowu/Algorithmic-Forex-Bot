...
def _fetch_headlines_av(symbol: str, limit: int = 10) -> list[str]:
    ...
    try:
        ...
        data = resp.json()
        feed = data.get("feed", [])
        return [item.get("title", "") for item in feed if item.get("title")]
    except Exception as exc:
        logger.warning(f"[SentimentAnalyst] AV news fetch failed for {symbol}: {exc}")
        return []


def _fetch_headlines_newsapi(symbol: str, limit: int = 10) -> list[str]:
...
def _fetch_headlines_finnhub(symbol: str, limit: int = 15) -> list[str]:
...
class SentimentAnalyst(BaseAgent):
    ...
    def _analyse(self, symbol: str, engine: str) -> dict:
        ...
        seen = set()
        headlines = [h for h in headlines if not (h in seen or seen.add(h))]
        ...
        if final_engine == "groq":
            score = _score_groq(headlines, symbol)
        else:
            score = _score_vader(headlines)
        ...
        reasoning = f"Processed {len(headlines)} headlines. Top: '{headlines[0][:60]}...'"
